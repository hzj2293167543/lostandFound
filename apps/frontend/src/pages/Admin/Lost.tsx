...
export default function AdminLost() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await lostApi.getLostItems();
        setItems(data);
      } catch (error) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await lostApi.deleteLostItem(id);
  ...
  return (
    <div>
      ...
      <div className="space-y-4">
            ...
            <CardContent>
              <div className="text-sm text-gray-600 space-y-1">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <span>状态:</span>
                  <Badge variant={item.status.code === 0 ? 'destructive' : 'secondary'}>
                    {item.status.name}
                  </Badge>
                </div>
              </div>
        ...
        {items.length === 0 && <p className="text-gray-500">暂无失物记录</p>}
      </div>
    </div>
  );
}
