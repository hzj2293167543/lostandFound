...
export function useRepliesToMeInfinite(userId: number) {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isFetching, status } =
    useInfiniteQuery<PageResponse<CommentItem>>({
      queryKey: commentKeys.repliesToMe(userId),
      queryFn: ({ pageParam = 1 }) => commentApi.getRepliesToMe(userId, pageParam as number, 10),
