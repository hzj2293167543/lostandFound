...
// 参数一多，极容易漏掉、写错，且无法复用handleUserReport( @Body('status') status: string,
@Body('result') result: string, @Body('type') type: number) 正确示例 (必须遵循):
// 整体接收，类型安全，复用 shared 中的 Schema handleUserReport( @Body(new
